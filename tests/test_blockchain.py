...
def test_create_genesis_block(personal_blockchain):
    ...
    assert "signature" in personal_blockchain.chain[0].data


def test_add_block(personal_blockchain):
    personal_blockchain.add_block({"message": "Test Data"})
    assert len(personal_blockchain.chain) == 2
    assert personal_blockchain.chain[-1].data["data"] == {"message": "Test Data"}
    assert "signature" in personal_blockchain.chain[-1].data
...
def test_update_node_ip(personal_blockchain):
    personal_blockchain.add_trusted_node("TestNode", "contact", "192.168.1.1")
    personal_blockchain.update_node_ip("TestNode", "192.168.1.2")
    updated_node = next((node for node in personal_blockchain.trusted_nodes if node.node_id == "TestNode"), None)
    assert updated_node is not None
    assert updated_node.ip_address == "192.168.1.2"
...
async def test_restore_from_backup(personal_blockchain):
    ...
    mock_blockchain.add_block({"message": "Test Block"})
    n, k = 5, 3
    ...
    try:
        ...
        assert reconstructed_blockchain.chain[-1].data["data"] == mock_blockchain.chain[-1].data["data"]
